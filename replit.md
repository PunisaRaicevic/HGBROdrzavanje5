...
### Database Migration: Supabase to Drizzle ORM (November 11, 2025)

**Complete migration from Supabase to Replit PostgreSQL with Drizzle ORM**
- **Objective**: Eliminate dependency on external Supabase service and use Replit's built-in PostgreSQL database
- **Implementation**:
  - Created `server/db.ts` with Drizzle PostgreSQL client using DATABASE_URL
  - Migrated all Supabase queries to Drizzle ORM query builder
  - Replaced `SupabaseStorage` class with `DrizzleStorage` class in `server/storage.ts`
  - Generated and applied database migrations using drizzle-kit
  - Removed `server/lib/supabase.ts` and all @supabase/supabase-js dependencies
  - Auto-generated SESSION_SECRET for development mode (warns that sessions reset on server restart)
- **Database Setup**:
  - All tables (users, tasks, task_history, notifications) migrated to Replit PostgreSQL
  - Schema definitions remain in `shared/schema.ts` using Drizzle column types
  - Database accessed via standard PostgreSQL connection (DATABASE_URL)
  - Sessions stored in Neon PostgreSQL (separate DATABASE_URL) via connect-pg-simple
- **Impact**: Application now fully self-contained on Replit infrastructure, no external database dependencies
...
**Primary Database**: Replit PostgreSQL (DATABASE_URL)

**Database Client**: Drizzle ORM with PostgreSQL driver for all data operations

**Session Storage**: Neon PostgreSQL (separate DATABASE_URL) via connect-pg-simple for Express session persistence

**Implementation**: 
- `server/db.ts` exports Drizzle PostgreSQL client and schema table definitions
- `server/storage.ts` implements `DrizzleStorage` class using Drizzle query builder
- Type definitions in `shared/schema.ts` for TypeScript type safety
- Database migrations in `migrations/` folder generated by drizzle-kit
- Environment variables: DATABASE_URL (required), SESSION_SECRET (auto-generated in dev mode)
...
- session - Express session store (stored in Neon PostgreSQL via separate DATABASE_URL)
...
**Database**: 
- Replit PostgreSQL for primary data storage (tasks, users, history, notifications)
- Neon Serverless PostgreSQL for session storage only (separate DATABASE_URL)
- Drizzle ORM (@drizzle-team/drizzle-orm) for database operations
- drizzle-kit for schema migrations
- postgres driver for PostgreSQL connections
