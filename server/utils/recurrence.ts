...
export function shouldProcessRecurringTask(
  ...
  // Check if the scheduled occurrence is within the recurrence period
  // This ensures tasks are created even if cron runs late
  if (recurrenceEndDate) {
    const endDate = new Date(recurrenceEndDate);
    // Process if the scheduled occurrence was before the end date
    // (not if NOW is before end date, to handle delayed cron execution)
    if (nextDate > endDate) {
      return false;
    }
  }
 ...
 * @param recurrenceEndDate - Optional end date for recurrence
 * @returns true if recurrence should continue
 */
